...
@router.get("/{studentID}",response_model = StudentRead)
def getstudent(studentID:int,db:Session = Depends(get_db)):
    return read_student(db,studentID)
...
@router.patch("/{studentID}",response_model=StudentUpdate)
